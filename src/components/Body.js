...
// if online return online page
  return (allrestro.length == 0) ? <Shimm /> : (
    <div className="mt-10">
      ...
      <div className="relative mx-[210px] mr-[240px] my-[60px]">
        ...
        <input type="text"  className="block w-full p-4 pl-10 text-sm border border-gray-300 rounded-lg bg-gray-50 " placeholder="Search for restaurants and foods" value={searchValue}
          onChange={(e) => {
            setsearchValue(e.target.value);
          }}></input>
        <button type="submit" className="text-white absolute right-2.5 bottom-2.5 bg-red-700 hover:bg-red-800  font-medium rounded-lg text-sm px-4 py-2 " onClick={() => {
      ...
      <div className="flex flex-wrap gap-[30px] px-[210px] ">
        {
          filteredrestaurant.length == 0 ? <h1>not found</h1> : filteredrestaurant.map((restaurant) => {
